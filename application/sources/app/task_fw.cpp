...
void task_fw(ak_msg_t *msg)
{
	switch (msg->sig)
	{
		...
		// 	}
		// 	/* otify update app firmware completed */
	...
	case FW_UPDATE_REQ:
	{
		...
		APP_DBG("[MCU]fw_header_file.checksum:%04X\n", firmware_header_file.checksum);
		APP_DBG("[MCU]fw_header_file.bin_len:%d\n", firmware_header_file.bin_len);
		APP_DBG("MCU sizeof(firmware_header_t) = %d\n", sizeof(firmware_header_t));
	...
	case FW_TRANSFER_REQ:
	{
		...
		APP_DBG("[MCU]Rx len=%d, bin_file_cursor=%d\n", firmware_packet_len, bin_file_cursor);
		APP_DBG("[MCU] start write\r\n");
		...
		if (bin_file_cursor >= firmware_header_file.bin_len) {
			...
			APP_DBG_SIG("FW_TRANSFER_REQ COMPLETED\n");
			task_post_pure_msg(AC_TASK_FW_ID, FW_CHECK_FCS);
		}
		else {
		uarrt_tx_ack();
		}
	...
	case FW_CHECK_FCS:
	{
		...
		uint16_t checksum_calculated = calc_checksum_ext_flash (APP_FLASH_FIRMWARE_START_ADDR, firmware_header_file.bin_len);
	...
	case FW_PACKED_TIMEOUT:
	{
		...
		shell_fwset_status = false;
		// task_post_pure_msg(AC_TASK_SHELL_ID,AC_SHELL_RESET_CMD);
