...
uint8_t nRF24_ReadReg(uint8_t reg) {
	...
	#else
	SPI.transfer16(reg & 0x1f); // Select register to read from
	value = SPI.transfer16(nRF24_CMD_NOP); // Read register value
	#endif
	nRF24_CSN_H();
...
void nRF24_ReadBuf(uint8_t reg, uint8_t *pBuf, uint8_t count) {
	...
	#else
	SPI.transfer16(reg); // Select register to read from
	while (count--) *pBuf++ = SPI.transfer16(nRF24_CMD_NOP);
	#endif
	nRF24_CSN_H();
}
...
void nRF24_WriteBuf(uint8_t reg, uint8_t *pBuf, uint8_t count) {
	...
	#else
	SPI.transfer16(reg); // Select register
	while (count--) SPI.transfer16(*pBuf++);
	#endif
	nRF24_CSN_H();
}
